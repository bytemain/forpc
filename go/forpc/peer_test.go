...
func TestEncodeBatchSinglePacket(t *testing.T) {
	pkt := Packet{StreamID: 42, Kind: FrameData, Payload: []byte("single")}
	single, err := pkt.Encode()
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	batch, err := EncodeBatch([]Packet{pkt})
	if err != nil {
		t.Fatalf("encode batch: %v", err)
	}
	if string(single) != string(batch) {
		t.Fatalf("single packet batch should equal normal encode")
	}
}

func TestEncodeBatchRoundtrip(t *testing.T) {
	hdr, _ := proto.Marshal(&pb.Call{Method: "test", Metadata: nil})
	tr, _ := proto.Marshal(&pb.Status{Code: pb.StatusCode_OK, Message: "OK"})
	packets := []Packet{
		{StreamID: 1, Kind: FrameHeaders, Payload: hdr},
		{StreamID: 1, Kind: FrameData, Payload: []byte("payload")},
		{StreamID: 1, Kind: FrameTrailers, Payload: tr},
	}
	encoded, err := EncodeBatch(packets)
	if err != nil {
		t.Fatalf("encode batch: %v", err)
	}
	// Batch should start with sentinel stream_id=0
	if binary.BigEndian.Uint32(encoded[:4]) != 0 {
		t.Fatal("expected batch sentinel 0x00000000")
	}
	decoded, err := DecodePackets(encoded)
	if err != nil {
	...
	if decoded[0].Kind != FrameHeaders || decoded[0].StreamID != 1 {
		t.Fatalf("packet 0: expected HEADERS stream=1, got kind=%d stream=%d", decoded[0].Kind, decoded[0].StreamID)
	}
	if decoded[1].Kind != FrameData || string(decoded[1].Payload) != "payload" {
		t.Fatalf("packet 1: expected DATA 'payload', got kind=%d payload=%q", decoded[1].Kind, decoded[1].Payload)
	}
	if decoded[2].Kind != FrameTrailers {
		t.Fatalf("packet 2: expected TRAILERS, got kind=%d", decoded[2].Kind)
	}
}

func TestDecodePacketsSinglePacket(t *testing.T) {
	pkt := Packet{StreamID: 99, Kind: FrameData, Payload: []byte("solo")}
	...
	if decoded[0].StreamID != 99 || string(decoded[0].Payload) != "solo" {
		t.Fatalf("unexpected packet: stream=%d payload=%q", decoded[0].StreamID, decoded[0].Payload)
	}
}
